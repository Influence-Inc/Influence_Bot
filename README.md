...
- **Team stays notified of chat activity** — Every creator/brand chat message pings `#content-reviews`, threaded under the review post so the conversation stays grouped. Because Slack doesn't notify anyone of a thread reply they aren't following, inbound (creator/brand) messages are also broadcast to the channel so the team actually sees them; the team's own admin-sent replies stay quiet threaded posts. Set `SLACK_REVIEWS_NOTIFY` (a user-group, user, or `<!here>`/`<!channel>` mention) to add a hard ping on inbound messages even when the channel is muted
